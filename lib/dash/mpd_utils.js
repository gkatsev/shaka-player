...
shaka.dash.MpdUtils = class {
   ...
   * @return {Array.<shaka.dash.MpdUtils.InsertionAction>} actions
   */
  static parseInsertion(insertion) {
    if (!insertion) {
      return [];
    }

    const regex = /(([x+])([1-9][0-9]*))/;
    if (!regex.test(insertion)) {
      console.error('Trying to parse invalid insertion!');
      return [];
    }
  ...
  static createTimeline(
    ...
    for (let i = 0; i < timePoints.length; ++i) {
      ...
      const insertion = timePoint.getAttribute('i');
      const insertionActions = shaka.dash.MpdUtils.parseInsertion(insertion);
      ...
      for (let j = 0; j <= repeat; ++j) {
        ...
        timeline.push(item);
        insertionTimeline.push(item);

        startTime = endTime;
        lastEndTime = endTime;
      }

      if (insertionActions.length) {
        let actions = insertionActions.slice();
        ...
        while (actions.length) {
          ...
          switch (action.action) {
            case 'repeat':
              for (let k = 0; k < action.count - 1; k++) {
                const prev = insertionTimeline[insertionTimeline.length - 1];
                const startTime = prev.end * timescale;
                const endTime = startTime + d;
                const item = {
                  repeated: 1,
                  start: startTime / timescale,
            ...
            case 'skip':
            {
              for (let k = 1; k < action.count; k++) {
                const timePoint = timePoints[i + k];
                let t = XmlUtils.parseAttr(
                    timePoint, 't', XmlUtils.parseNonNegativeInt);
                const d = XmlUtils.parseAttr(
                    timePoint, 'd', XmlUtils.parseNonNegativeInt);
                if (t != null) {
                  t -= unscaledPresentationTimeOffset;
                }
    ...
    return insertionTimeline.sort((a, b) => a.start - b.start);
    // return timeline;
  }
 ...
 * }}
 */
shaka.dash.MpdUtils.InsertionAction;
